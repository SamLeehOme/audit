	...
	Paging.prototype = {
		init: function(settings) {
			this.settings = $.extend({
				...
				toolbar: false,
				hash:true,
				pageSizeList: [10, 50, 100, 500, 1000]
			}, settings);
			...
			this.container.on('click', 'li.js-page-action,li.ui-pager', function(e) {
				...
				if ($(this).hasClass('js-page-action')) {
					...
					if ($(this).hasClass('js-page-next')) {
						...
						$("#cpageNum").val(cpage);//这个是下页的页数，也就是加过之后的页数
						getData();
						_this.current = Math.min(_this.pagecount, _this.current + 1);
					}
					if ($(this).hasClass('js-page-last')) {
						var totalPage = Number($("#totalPage").html());
						$("#cpageNum").val(totalPage);
						getData();
						_this.current = _this.pagecount;
					}
				} else if ($(this).data('page')) {
					var cpage=$("#jumpNumTxt").val();//当前页为我们要跳转的页面
					var totalPage = Number($("#totalPage").html());
					if(cpage>=totalPage){
						cpage=totalPage;
					}
					$("#cpageNum").val($(this).data('page'));
					getData();
		...
		bindToolbar: function() {
			...
			$('a', html).click(function() {
				...
				$("#cpageNum").val(current);
				getData();
				$('.ui-paging-count').val(current);
				_this.go(current);
			});
			sel.change(function() {
				$("#pageSize").val(Number($(this).val()));
				$("#cpageNum").val(0);
				getData();
